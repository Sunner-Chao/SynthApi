...
// DoFunc executes an HTTP request using the caller's configured proxy and
// connection pool.
type DoFunc func(*http.Request) (*http.Response, error)

// Channel caches the attested server key for one account. A changed access
// point or API key invalidates the cache and triggers a new negotiation.
type Channel struct {
	mu                sync.Mutex
	baseOrigin        string
...
// Do encrypts the request body and decrypts the response according to the
// AICC secure-channel protocol used by the official Seedance SDK.
func (c *Channel) Do(
	ctx context.Context,
	req *http.Request,
...
func encryptRequest(plaintext []byte, publicKey *rsa.PublicKey) ([]byte, []byte, error) {
	...
	message, err := json.Marshal(encryptedMessage{
		Nonce: nonce, MAC: mac, Key: encryptedKey, Ciphertext: ciphertext,
	})
	if err != nil {
		return nil, nil, err
	}
