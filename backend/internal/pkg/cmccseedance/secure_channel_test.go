...
func TestChannelSecureRequestResponseRoundTripAndCachesAttestation(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			_ = json.NewEncoder(w).Encode(map[string]any{
				"Result": map[string]any{
					"attestation": map[string]any{
						"key_info": map[string]any{"pub_key_info": publicPEM},
					},
				},
			})
		case "/api/v3/contents/generations/tasks":
			...
			_ = json.NewEncoder(w).Encode(encryptedMessage{Nonce: nonce, MAC: mac, Ciphertext: ciphertext})
		default:
			http.NotFound(w, r)
		}
	}))
...
func TestChannelPassesThroughPlaintextErrorResponses(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/v1/security/token" {
			_ = json.NewEncoder(w).Encode(map[string]any{"pub_key_info": publicPEM})
			return
		}
