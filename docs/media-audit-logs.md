...
Nginx deployments with a five-second generic `/api/` timeout must include `scripts/maintenance/nginx/synthapi-media-audit-{public|admin}.conf` in the corresponding virtual hosts. The dedicated authenticated media location streams results without buffering and allows up to 90 seconds for the upstream response; database queries remain bounded at 12 seconds in Go.
