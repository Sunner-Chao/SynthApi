 ...
 */
// Version key to force cache refresh when currency settings change
const STORAGE_VERSION = 2

function getStoredVersion(): number {
  try {
    return Number(localStorage.getItem('system-config-version') || '0')
  } catch {
    return 0
  }
}
...
if (getStoredVersion() < STORAGE_VERSION) {
  try {
    ...
    localStorage.setItem('system-config-version', String(STORAGE_VERSION))
  } catch {
    /* empty */
  }
}
