...
const getPriceSummary = (row: ModelRow, t: (key: string) => string) => {
  ...
  // 使用 formatBillingCurrencyFromUSD 格式化价格（model_ratio 已经是每百万 token 的价格）
  const formattedPrice = formatBillingCurrencyFromUSD(Number(inputPrice))

  return extraCount > 0
    ? `${t('Input')} ${formattedPrice} · ${extraCount} ${t('extras')}`
    : `${t('Input')} ${formattedPrice}`
}
