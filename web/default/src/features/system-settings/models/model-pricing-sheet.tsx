  ...
  return [
    {
      ...
      label: t('Input price'),
      value: promptPrice
        ? formatBillingCurrencyFromUSD(Number(promptPrice))
        : t('Empty'),
    },
    {
      key: 'completion',
      ...
      value:
        laneEnabled.completion && lanePrices.completion
          ? formatBillingCurrencyFromUSD(Number(lanePrices.completion))
          : t('Empty'),
    },
    {
      key: 'cache',
      ...
      value:
        laneEnabled.cache && lanePrices.cache
          ? formatBillingCurrencyFromUSD(Number(lanePrices.cache))
          : t('Empty'),
    },
    {
      key: 'createCache',
      ...
      value:
        laneEnabled.createCache && lanePrices.createCache
          ? formatBillingCurrencyFromUSD(Number(lanePrices.createCache))
          : t('Empty'),
    },
    {
      key: 'image',
      ...
      value:
        laneEnabled.image && lanePrices.image
          ? formatBillingCurrencyFromUSD(Number(lanePrices.image))
          : t('Empty'),
    },
    {
      key: 'audio',
      ...
      value:
        laneEnabled.audioInput && lanePrices.audioInput
          ? formatBillingCurrencyFromUSD(Number(lanePrices.audioInput))
          : t('Empty'),
    },
    {
      key: 'audioCompletion',
      ...
      value:
        laneEnabled.audioOutput && lanePrices.audioOutput
          ? formatBillingCurrencyFromUSD(Number(lanePrices.audioOutput))
          : t('Empty'),
    },
  ]
}
...
function PriceInput(props: {
  ...
  /** If true, value is in USD and should be displayed in local currency */
  isUSD?: boolean
}) {
  const { config } = getCurrencyDisplay()
  const symbol = config.quotaDisplayType === 'CNY' ? '¥' : '$'
  const exchangeRate =
    config.quotaDisplayType === 'CNY' ? config.usdExchangeRate : 1

  // Local state for display value - preserves user input including decimal points
  const [displayValue, setDisplayValue] = useState<string>('')

  // Sync from props when value changes externally
  useEffect(() => {
    if (props.value && props.isUSD) {
      setDisplayValue(
        String(roundToDecimals(Number(props.value) * exchangeRate, 2))
      )
    } else if (props.value !== undefined) {
      setDisplayValue(props.value)
    }
  }, [props.value, props.isUSD, exchangeRate])

  // Handle input change - update display immediately, convert on blur
  ...
  // Convert to USD on blur
  const handleBlur = () => {
    if (props.isUSD && displayValue) {
      const usdValue = roundToDecimals(Number(displayValue) / exchangeRate, 6)
      props.onChange(String(usdValue))
    } else {
