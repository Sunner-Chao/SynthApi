...
export function getApiKeyFormSchema(
  ...
  return z
    ...
    .superRefine((data, ctx) => {
      if (data.group === 'auto') {
        if (data.auto_groups_mode === 'inherit' && !hasGlobalAutoGroups) {
          ctx.addIssue({
            code: 'custom',
            path: ['auto_groups'],
            message: t(
              'Select at least one Auto group because no global Auto order is configured.'
            ),
          })
        }
...
export function getApiKeyFormDefaultValues(
  ...
  return {
    ...
    auto_groups_mode: 'inherit',
    auto_groups: [],
    cross_group_retry: defaultUseAutoGroup && autoCrossGroupRetryEnabled,
  }
}
...
export function transformApiKeyToFormDefaults(
  ...
  const autoGroupsMode =
    storedAutoGroups.length > 0 || !hasGlobalAutoGroups ? 'custom' : 'inherit'
