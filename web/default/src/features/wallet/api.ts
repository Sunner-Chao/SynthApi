...
/**
 * Request QR code payment (creates pending order)
 */
export async function requestQRCodePayment(
  request: PaymentRequest
): Promise<ApiResponse<{ trade_no: string; amount: number; money: number }>> {
  const res = await api.post('/api/user/qrcode/pay', request, {
    skipBusinessError: true,
  } as Record<string, unknown>)
  return res.data
}
...
/**
 * Get XPay order status
 */
export async function getXPayOrderStatus(tradeNo: string): Promise<
  ApiResponse<{
    trade_no: string
    amount: number
    payment_method: string
...
> {
  const res = await api.get(`/api/user/xpay/status/${tradeNo}`)
  return res.data
}

/**
 * Get user's XPay orders
 */
export async function getXPayOrders(
  page: number = 1,
  pageSize: number = 10
): Promise<
  ApiResponse<{
...
> {
  const res = await api.get(
    `/api/user/xpay/orders?page=${page}&page_size=${pageSize}`
  )
  return res.data
}
