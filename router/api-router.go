...
func SetApiRouter(router *gin.Engine) {
	...
	apiRouter.Use(middleware.GlobalAPIRateLimit())
	{
		...
		apiRouter.GET("/admin/r2-monitor", middleware.AdminAuth(), controller.GetR2Monitor)
		...
		userRoute := apiRouter.Group("/user")
		{
			...
			adminRoute.Use(middleware.AdminAuth())
			{
				...
				adminRoute.POST("/xpay/confirm/:trade_no", controller.XPayConfirmOrder)
		...
		subscriptionAdminRoute.Use(middleware.AdminAuth())
		{
			...
			subscriptionAdminRoute.POST("/plans/unlimited-presets", controller.AdminEnsureUnlimitedSubscriptionPlanPresets)
		...
		// Custom OAuth provider management (all administrators)
		...
		tokenRoute.Use(middleware.UserAuth())
		{
			...
			tokenRoute.POST("/:id/key", middleware.CredentialReadRateLimit(), middleware.DisableCache(), controller.GetTokenKey)
			tokenRoute.POST("/", controller.AddToken)
			tokenRoute.PUT("/", controller.UpdateToken)
			tokenRoute.DELETE("/:id", controller.DeleteToken)
			tokenRoute.POST("/batch", controller.DeleteTokenBatch)
			tokenRoute.POST("/batch/keys", middleware.CredentialReadRateLimit(), middleware.DisableCache(), controller.GetTokenKeysBatch)
		}
		...
		registerMediaAuditRoutes(apiRouter)
