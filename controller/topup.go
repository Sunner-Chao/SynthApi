...
// GetEpayV2Client 获取易支付 V2 客户端 (RSA 签名)
func GetEpayV2Client() *common.EpayV2Client {
	if operation_setting.PayAddress == "" || operation_setting.EpayId == "" {
		return nil
	}

	// 检查是否配置了 RSA 密钥
	privateKey := operation_setting.EpayPrivateKey
	publicKey := operation_setting.EpayPublicKey

	if privateKey == "" || publicKey == "" {
		return nil
	}

	return common.NewEpayV2Client(&common.EpayV2Config{
		MerchantNo:  operation_setting.EpayId,
...
func RequestEpay(c *gin.Context) {
	...
	// 尝试使用 V2 客户端 (RSA 签名)
	v2Client := GetEpayV2Client()
	var uri string
	var params map[string]string

	if v2Client != nil {
		// 使用 V2 RSA 签名
		params, err = v2Client.CreateOrder(map[string]string{
		...
		if err != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("易支付V2 签名失败 user_id=%d trade_no=%s error=%q", id, tradeNo, err.Error()))
			c.JSON(http.StatusOK, gin.H{"message": "error", "data": "签名失败"})
			return
		}
		uri = operation_setting.PayAddress + "/api/pay/create"
	} else {
		// 使用 V1 MD5 签名
...
func EpayNotify(c *gin.Context) {
	...
	if v2Client != nil {
		// 使用 V2 RSA 验证
		isValid, verifyErr := v2Client.VerifyCallback(params)
		if verifyErr == nil && isValid {
			// V2 验证成功，构造 VerifyRes
...
// QRCodePaymentRequest represents a QR code payment request
type QRCodePaymentRequest struct {
	Amount        int64  `json:"amount"`
	PaymentMethod string `json:"payment_method"`
}

// RequestQRCodePayment creates a pending order for QR code payment
func RequestQRCodePayment(c *gin.Context) {
	var req QRCodePaymentRequest
	...
	topUp := &model.TopUp{
		...
		TradeNo:         tradeNo,
		PaymentMethod:   req.PaymentMethod,
		PaymentProvider: model.PaymentProviderEpay, // Use EPay provider for compatibility
		CreateTime:      time.Now().Unix(),
		Status:          common.TopUpStatusPending,
	}

	if err := topUp.Insert(); err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("创建充值订单失败 user_id=%d trade_no=%s error=%q", id, tradeNo, err.Error()))
		common.ApiErrorMsg(c, "创建订单失败")
		return
	}

	logger.LogInfo(c.Request.Context(), fmt.Sprintf("二维码充值订单创建成功 user_id=%d trade_no=%s amount=%d money=%.2f", id, tradeNo, req.Amount, payMoney))
