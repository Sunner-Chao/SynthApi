...
type User struct {
	...
	ModelRequestMaxConcurrency int            `json:"model_request_max_concurrency,omitempty" gorm:"-"`
...
// GetUserByIdIncludeDeleted 根据 ID 获取用户（包括已注销/软删除的用户）
func GetUserByIdIncludeDeleted(id int, selectAll bool) (*User, error) {
	if id == 0 {
		return nil, errors.New("id 为空！")
	}
	user := User{Id: id}
